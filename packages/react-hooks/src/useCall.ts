...
function extractParams (fn: any, params: any[], paramMap: (params: any[]) => any): [string, CallParams | null] {
  return [
    JSON.stringify({ f: fn?.name || fn, p: params }),
...
function subscribe <T> (appReadyStatus: boolean, tracker: TrackerRef, fn: TrackFn | undefined, params: CallParams, setValue: (value: T) => void, { isSingle, transform = transformIdentity, withParams }: CallOptions<T>): void {
    ...
    if (appReadyStatus) {
      if (fn && (!fn.meta || !fn.meta.type?.isDoubleMap || validParams.length === 2)) {
        ...
        tracker.current.subscriber = fn(...params, (value: any): void => {
          ...
          if (appReadyStatus && tracker.current.isActive && (!isSingle || !tracker.current.count)) {
            tracker.current.count++;
            appReadyStatus && tracker.current.isActive && setValue(
              withParams
  ...
  useEffect((): void => {
    ...
    if (appReadyStatus && fn) {
      ...
      if (mappedParams && serialized !== tracker.current.serialized) {
        ...
        subscribe(appReadyStatus, tracker, fn, mappedParams, setValue, options);
      }
    }
  }, [fn, options, appReadyStatus, params]);
